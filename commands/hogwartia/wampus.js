...
module.exports = class HornedSerpentCommand extends Command {
  constructor(stevebot) {
    super(stevebot, {
      ...
      group: 'hogwartia',
      memberName: 'wampus',
      description: 'Adds a member to the Wampus role'
    });
  }
  ...
  run(msg) {
    ...
    if (member.roles.get(wampus.id)) return msg.reply('you\'re already in that house.');
    if (member.roles.get(thunderbird.id) || member.roles.get(hornedserpent.id) || member.roles.get(pukwudgie.id)) return msg.reply('you\'re already in an Ilvermorny house. Don\'t be that person.');
